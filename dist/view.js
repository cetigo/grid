...
var View = function () {
    ...
    _createClass(View, [{
        ...
        key: 'getOffsetForPanel',
        value: function getOffsetForPanel(panelIndex) {
            var rowIndex = Math.ceil((panelIndex + 1) / this.hardware.panelsPerRow);
        ...
        key: 'appendPanel',
        value: function appendPanel(options) {
        ...
        key: 'resetView',
        value: function resetView(options) {
            options = options || {};

            // this is effectively just core.view.reset() plus some target variables
            systemController.resetView();
            $('#results').html('').append(options.loaderHtml || '');
        }
    }, {
        key: 'initialize',
        value: function initialize(noPositionReset) {
            $('#loader').remove();
        ...
        key: 'checkIfFilterApplies',
        value: function checkIfFilterApplies(options) {
        ...
        key: 'filterCurrentPanel',
        value: function filterCurrentPanel() {
            if (this.album.active || !this.slideshow.active && !this.slideshow.highlightingActive) return;

            var gatheredUrl = void 0;
            if (this.mode.reel) gatheredUrl = reelController.vars.reel[reelController.vars.position].rawUrl;else {
        ...
        key: 'appendCards',
        value: function appendCards(cardData) {
            ...
            for (var i = 0, len = cardData.length; i < len; i++) {
                ...
                }).html(cardData[i].html).appendTo('#results');
            }
        }
        ...
        key: 'openSlideshow',
        value: function openSlideshow(position) {
        ...
        key: 'switchSlideshow',
        value: function switchSlideshow(backwards, noCancel) {
        ...
        key: 'revealProximity',
        value: function revealProximity() {
            for (var i = 5; i >= -5; i--) {
                var targetPanel = this.panels[this.slideshow.position + i];
                if (targetPanel) targetPanel.reveal();
            }
        }
    }, {
        key: 'quitSlideshow',
        value: function quitSlideshow() {
            if (this.album.active) {
                return;
            }
        ...
        key: 'showMetadata',
        value: function showMetadata() {

            // Gather information for metadata labels
            var slideshowObject = this.activePanel;
            var metaUrlArray = slideshowObject.downloadUrl.split('/');
            var metaFileType = metaUrlArray[metaUrlArray.length - 1].split('.')[1];
            var metaPanelType = slideshowObject.contentType === 2 ? "video" : "image";
            var metaDimensions = slideshowObject.metaData.dimensions.width ? slideshowObject.metaData.dimensions.width + "x" + slideshowObject.metaData.dimensions.height : "loading";

            // Wrapper to attach blocks to
            var metaWrapElem = $('<div/>');

            // Block 1 - Core
            $('<div/>', { class: 'metablock', id: 'meta-base' }).html(metaFileType + " " + metaPanelType + "<br>" + metaDimensions + "<br>" + (this.slideshow.position + 1) + " / " + this.slideshow.length).appendTo(metaWrapElem);
            ...
            if (slideshowObject.contentType === 2) {
                ...
                var metaVideoElem = $('<div/>', { class: 'metablock ', id: 'meta-video' }).html("Video loading<br>" + videoHostname).appendTo(metaWrapElem);

                var metaVideoObject = $('#slideshow video').get(0);

                var videoUpdateHandler = function videoUpdateHandler() {

                    var hasAudio = !!this.mozHasAudio || !!this.webkitAudioDecodedByteCount || this.audioTracks && !!this.audioTracks.length;
                    slideshowObject.videoUrl.hasAudio = hasAudio;
                    var audioString = hasAudio ? "sound" : "mute";
                    var currentSrcUrl = new URL(this.currentSrc);

                    metaVideoElem.html(Math.round(this.currentTime) + 's/' + Math.round(this.duration) + 's<br>\n                        ' + audioString + '<br>\n                        ' + currentSrcUrl.hostname);
                };
            ...
            if (slideshowObject.fetch.type !== 'blank') {
                ...
                var metaFetchStatus = slideshowObject.fetch.required ? 'loading' : 'fetched';

                $('<div/>', { class: 'metablock', id: 'meta-fetch' }).html(slideshowObject.fetch.type + "<br>" + metaFetchStatus + "<br>" + truncateString(slideshowObject.fetch.data, 15)).appendTo(metaWrapElem);
            }

            // Block 4 - Title, Score/Author, Age [optional]
            if (slideshowObject.title) {
                var scoreOrAuthor = slideshowObject.metaData.score || slideshowObject.metaData.author || "";

                $('<div/>', { class: 'metablock', id: 'meta-advanced' }).html(truncateString(slideshowObject.title, 60) + "<br>" + scoreOrAuthor + "<br>" + slideshowObject.metaData.age).appendTo(metaWrapElem);
            }
            ...
            if (slideshowObject.metaData.extended) {
                ...
                $('<div/>', { class: 'metablock', id: 'meta-extended' }).html(extendedMetaHtml).appendTo(metaWrapElem);
            }

            var classString = settings.showHelp ? "active helpactive" : "active";

            $('#meta').addClass(classString).html('');
        ...
        value: function updateControls() {
            ...
            if (!settings.showHelp) {
                ...
                if (system.customMode) $('#controlblock-save-label').text('Remove');else $('#controlblock-save-label').text('Save');
            }
        ...
        key: 'skimVideo',
        value: function skimVideo(time) {
            if (this.slideshow.active) this.activePanel.skimVideo(time);
        }
    }, {
        key: 'inspectSlideshow',
        value: function inspectSlideshow() {
        ...
        key: 'toggleVideoControlMode',
        value: function toggleVideoControlMode(forceOff) {
        ...
        value: function activateHighlight(skipFocus) {
        ...
        value: function disableHighlight() {
            this.slideshow.highlightingActive = false;
            helpController.refresh();
            core.input.focus();
            $('.focus').removeClass('focus');
        ...
        key: 'focusHighlight',
        value: function focusHighlight(overwriteIndex) {
