...
var commands = {
        ...
        method: function method(args) {
            ...
            if (args[1]) {
                reelController.initialize({
                    ...
                    sources: args.slice(1).join(' ').split(',').map(function (s) {
                        return s.trim();
                    })
                });
            } else {
    ...
    "": {
        method: function method(args) {
            ...
            $.each(sortedFavorites, function (i, obj) {
                ...
                }).html('<div class="panel-label">' + obj + '</div><div class="panel-background" style="background-image: url(' + favorites[obj].thumb + ')"></div>').appendTo('#results');
                panelsToDraw--;
                panelsDrawn++;
            });
        ...
        method: function method(args) {
            ...
            if (args[1] == "1") {
                ...
                $.each(loadHandler, function (i, obj) {
                    ...
                    if (obj.filter) handlerListString += 'filter: <pre>' + obj.filter + '</pre><br>';else handlerListString += 'filter: none (default)<br>';
    ...
    "options": {
        method: function method(args) {
            ...
            }).html('<h1>Options</h1><br><br><div id="options-general"><h2>General</h2></div><div id="options-performance"><h2>Performance</h2></div>').appendTo('#results');
            ...
            $.each(generalDb.options.general, function (i, obj) {
                ...
                var newBox = $('<div/>', {
                    class: 'options-box',
                    id: 'optionbox-wrap-' + i
                }).html('<input type="checkbox" class="uiswitch" id="optionbox-' + i + '" ' + onclickString + '><h6>' + obj.label + '</h6>').appendTo(optionsGeneralElem);
            ...
            $.each(generalDb.options.performance, function (i, obj) {
                ...
                var newBox = $('<div/>', {
                    class: 'options-box',
                    id: 'optionbox-wrap-' + i
                }).html('<input type="checkbox" class="uiswitch" id="optionbox-' + i + '" ' + onclickString + '><h6>' + obj.label + '</h6>').appendTo(optionsPerformElem);
...
var customCommands = {
    ...
    "/clear": {
        method: function method(args) {
            for (var member in customMode.data) {
                delete customMode.data[member];
...
var reelController = {
    ...
    resetView: function resetView(loadingText1, loadingText2) {
        $('#reel #loader, #reel #videocnt').remove();
        $('#reel').css({
            "background-image": "none"
        }).append('<div id="loader">Loading Reel<br>' + loadingText1 + '<br>' + loadingText2 + '</div>').addClass('active');
    ...
    refillPool: function refillPool(callback) {
        var _this = this;
        ...
        $.each(chosenSources, function (i, source) {
            core.parser.workerInvoker({ input: source }).then(function (answer) {
                if (answer) _this.fillPoolWith(answer.data);
                if (callback) callback();
            });
        });
    },

    fillPoolWith: function fillPoolWith(panels) {
        this.vars.pool = this.vars.pool.concat(panels);
    },

    getPoolItems: function getPoolItems(amount) {
        var returnArray = [];
    ...
    extendReel: function extendReel(amount) {
        var newItems = this.getPoolItems(amount);
    ...
    getSources: function getSources(amount) {
        var shuffledSources = shuffleArray(this.vars.sources);
        var answerArray = [];
        for (var i = 0; i < amount; i++) {
...
function extendPage(invokeCallback) {
    ...
    core.parser.session.JSON = $.ajax({
        ...
        error: function error(x, status, _error2) {
            outputMessage("There was an error loading the page");
            $('.focus-nextload').removeClass('focus-nextload').addClass('focus-nextpage');
        }
...
function fetchJson(override, customSrcBlob, customType) {
    ...
    if (system.customMode) customModeParse(rawCommand, customSrcBlob, customType);else core.parser.parse(rawCommand);
}
...
var loadController = {
    ...
    updateProgress: function updateProgress(progress, type) {
...
function detectDataChildren(dataChildren) {
    for (var i = 0; i <= dataChildren.length - 1; i++) {
        ...
        detectUrl(dataChildren[i].data.url, {
    ...
    if (parseUrl.host === 'sli.mg' || parseUrl.host === 'i.sli.mg') {
        ...
        // Case i.sli.mg/ABCDE.xyz
            ...
            // Case sli.mg/ABCDE
                    ...
                    if (settings.limitImages) fragment += "l.jpg";else fragment += ".jpg";
...
function saveToCustom() {
    ...
    if (system.customMode) customController.removeImage(gatheredUrl);else customController.storeImage(gatheredUrl, gatheredTitle);
...
var customController = {
    ...
    leave: function leave(passthroughString) {
    ...
    renderOverview: function renderOverview() {
        ...
        $.each(customMode.backup, function (i, obj) {
            ...
            }).html('<div class="panel-label">Backup ' + String(i) + '<br><span style="color: ' + getRandomColor() + ';" class="prefixspan">' + elementCount + '</span> Elements</div>' + '<div class="panel-thumb-wrap">' + thumbElem + '</div>').appendTo('#results');
        });
    },

    /* -- Data -- */

    removeBlob: function removeBlob() {
        var remArr = [];
    ...
    storeImage: function storeImage(url, title) {
    ...
    findInData: function findInData(url) {
    ...
    explodeAlbums: function explodeAlbums() {
        $.each(core.view.panels, function (i, obj) {
            if (obj.albumArray) {
                var titleCount = 1;
...
function getThumbnails(data) {
    ...
    $.each(data, function (i, obj) {
        ...
        if (fileEnding === "jpg" || fileEnding === "jpeg" || fileEnding === "png" || fileEnding === "gif") {
...
function refreshNightMode() {
    if (settings.nightMode) $('body').addClass('nightmode');else $('body').removeClass('nightmode');
}

function outputMessage(string) {
    var additionalClass = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
...
var systemController = {
    ...
    setHash: function setHash(str) {
        this.vars.currentHash = str;
        window.location.hash = '#!/' + str;
    },

    readHash: function readHash(str) {
...
var uiController = {
    ...
    fillMenu: function fillMenu(type) {
        if (type == 1) {
    ...
    hideMenu: function hideMenu(silent) {
    ...
    switchMenuPosition: function switchMenuPosition(reverse) {
        if (reverse) {
            system.menuPosition--;
        } else {
            system.menuPosition++;
...
var contentController = {
    ...
    toggleOriginalZoom: function toggleOriginalZoom() {
